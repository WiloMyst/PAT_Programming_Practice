#include <iostream>//两次 Dijkstra+DFS
#include <algorithm>//最短路上由各顶点累积下来的量，往往Dijkstra中就能得到 
#include <vector>
using namespace std;
...
void dfsdispath(int v) {//因为只有唯一条路径，只走单支，深搜结束就完事 
    dispath.push_back(v);
    if(v == st) return ;
    dfsdispath(dispre[v]);
}
...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        visit[u] = true;
        for(int v = 0; v < n; v++) {
            if(visit[v] == false && e[u][v] != inf) {//第一次Dijkstra就可得到耗时最少的最短路前驱dispre 
                if(e[u][v] + dis[u] < dis[v]) {
                    dis[v] = e[u][v] + dis[u];//距离更新 
                    dispre[v] = u;
                    weight[v] = weight[u] + w[u][v];//耗时更新(起点到达该点的耗时) 
